...
async def doc(bot, update):
    ...
    if (_bool_metadata):
        ...
        if metadata:
            ...
            cmd = f"""ffmpeg -i "{path}"  -map 0 -c:s copy -c:a copy -c:v copy -metadata title="{metata}" -metadata author="{metdata}" -metadata:s:s title="{mtadaa}" -metadata:s:a title="{metadata}" -metadata:s:v title="{@Ani}"  "{metadata_path}" """
